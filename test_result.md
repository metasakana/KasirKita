...
## Session 2 - Stok pricing features (to test in iteration_5):
  6. Harga jual via persen markup (product-form.tsx): toggle "Rp"/"Naik %" (testIDs form-sellmode-rp / form-sellmode-persen). In % mode: percent input (form-pct-input), quick chips +5..+30% (form-pct-<v>), live computed text (form-sell-computed) "Harga jual: modal + X% = Rp Y" (sellPrice = round(cost*(1+pct/100))). Save uses computed value; validation: sellNum<=0 -> error toast "Harga jual belum diisi".
  7. Restock with variable harga beli (stok.tsx modal + StoreContext.restock): new optional input restock-cost-input. If filled & different from current costPrice -> creates SEPARATE product with same name/category but new costPrice (sellPrice copied) and info text restock-cost-info shown; if a same-name variant with that costPrice already exists -> qty merged into it. If empty/same -> normal qty increment. StockEntry now records costPrice, shown in stock-history as "· Modal Rp X".
